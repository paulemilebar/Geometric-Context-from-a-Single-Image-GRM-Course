...
def augment_features(feats: np.ndarray) -> np.ndarray:
    ...
    Returns (n_sp, 81) — 78 raw features + 3 AdaBoost class probabilities.
    The proba columns give the GNN a strong prior that it corrects via context.
    """
    if ADA_CLF is None:
...
def build_graph(ft_data: dict, sp_data: dict, device):
    ...
    feats81   = augment_features(feats)                   # (n, 81)
...
def main():
    ...
    print(f"adaboost prior : {'loaded ✓' if ADA_CLF is not None else 'NOT found — using zero proba features'}")
    ...
    for epoch in tqdm(range(1, EPOCHS + 1), desc="epochs"):
        ...
        if val_pix > best_val:
            best_val = val_pix
            torch.save(model.state_dict(), MODEL_DIR / "gnn_improved_best.pt")
    ...
    plot_curves(train_losses, train_accs, val_pix_accs,
                OUT_DIR / "step5b_training_curves.png")
    plot_predictions(model, ds, test_indices, device,
                     OUT_DIR / "step5b_predictions.png")

    torch.save(model.state_dict(), MODEL_DIR / "gnn_improved_final.pt")
    print(f"\nmodel saved to {MODEL_DIR}/gnn_improved_final.pt")
    print("done -> outputs/step5b_*.png")
