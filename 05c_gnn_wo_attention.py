...
def load_adaboost():
    ...
    if not p.exists():
        print("[warn] adaboost.pkl not found — using zeros for class-prob features")
        return None, None
    with open(p, "rb") as f:
        d = pickle.load(f)
    return d["clf"], d["scaler"]

ADA_CLF, ADA_SCALER = load_adaboost()

def augment_features(feats: np.ndarray) -> np.ndarray:
    if ADA_CLF is None:
        proba = np.zeros((len(feats), ADA_DIM), dtype=np.float32)
    else:
        X     = ADA_SCALER.transform(feats)
...
def build_graph(ft_data: dict, sp_data: dict, device):
    feats     = ft_data["features"].astype(np.float32)    
    sp_labels = ft_data["sp_labels"].astype(np.int64)     
    adj       = sp_data["adj"].astype(np.float32)         
    ...
    feats81   = augment_features(feats)                   
...
class GNNBlock(nn.Module):
    """GNN layer basique + residual connection + LayerNorm + ELU."""
    def __init__(self, dim: int, dropout: float):
        super().__init__()
        self.gnn  = SimpleGNNLayer(dim, dim, dropout)
        self.norm = nn.LayerNorm(dim)
        self.act  = nn.ELU()
        self.drop = dropout

    def forward(self, x, src_idx, dst_idx):
        h = self.gnn(x, src_idx, dst_idx)
        h = F.dropout(h, p=self.drop, training=self.training)
        return self.act(self.norm(x + h))          # residual


class ImprovedSimpleGNN(nn.Module):
    ...
    for start in range(0, len(idx), batch_size):
        ...
        for i in batch:
            ...
            if ft is None or sp is None:
                continue
            x, y, src, dst = build_graph(ft, sp, device)
...
def plot_predictions(model, ds, indices, device, save_path, n=6):
    ...
    fig.suptitle("Simple GNN predictions (No Attention)", fontsize=13)
...
def main():
    ...
    print(f"input : {IN_DIM}-dim  (78 raw features + 3 AdaBoost class probs)")
    print(f"adaboost prior : {'loaded ✓' if ADA_CLF is not None else 'NOT found'}")
    ...
    for epoch in tqdm(range(1, EPOCHS + 1), desc="epochs"):
        ...
        if val_pix > best_val:
            best_val = val_pix
            torch.save(model.state_dict(), MODEL_DIR / "gnn_simple_best.pt")
    ...
    model.load_state_dict(
        torch.load(MODEL_DIR / "gnn_simple_best.pt", map_location=device)
    )
    ...
    plot_curves(train_losses, train_accs, val_pix_accs,
                OUT_DIR / "step5c_training_curves.png")
    plot_predictions(model, ds, test_indices, device,
                     OUT_DIR / "step5c_predictions.png")

    torch.save(model.state_dict(), MODEL_DIR / "gnn_simple_final.pt")
    print(f"\nmodel saved to {MODEL_DIR}/gnn_simple_final.pt")
    print("done -> outputs/step5c_*.png")
